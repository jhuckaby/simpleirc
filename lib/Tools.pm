...
sub get_nice_date {
	...
	if ($yes_time) { $nice .= ' ' . get_nice_time($epoch, $yes_secs); }
...
sub get_nice_time {
	##
	# Given epoch, return pretty-printed time with optional seconds
	##
	my $epoch = shift;
	my $yes_secs = shift || 0;
	my $nice = '';
...
sub strip_channel {
	# strip leading hashmark from channel name (for internal commands)
	my $channel = trim( shift @_ );
	$channel =~ s/^\#//;
	return $channel;
}
sub sch { return strip_channel(@_); }

sub guess_content_type {
	# guess mime type for filename
	my $filename = basename(shift @_);
	my $mimetypes = MIME::Types->new;
	return $mimetypes->mimeTypeOf($filename) || 'text/plain';
}

sub remove_key_recursive {
	# recursively remove key from all hashes in tree
	my ($hash, $key) = @_;
	delete $hash->{$key};
	
	foreach my $key (keys %$hash) {
		if (ref($hash->{$key})) { remove_key_recursive($hash->{$key}, $key); }
	}
}
