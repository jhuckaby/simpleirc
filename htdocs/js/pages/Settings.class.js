Class.subclass( AppStr.Page.Base, "AppStr.Page.Settings", {	
	...
	receive_config: function(resp, tx) {
		...
		html += get_form_table_caption( "Optionally enter a global password to lock down the server, making it private.  This will be required by all users to connect, and is different from the username and password they use for identifying.");
		html += get_form_table_spacer('short transparent');
	...
	receive_remote_version: function(resp, tx) {
		...
		if (remote_version.BuildID != this.version.BuildID) {
			html += '<div class="info_value"><a href="http://pixlcore.com/software/simpleirc/changelog-'+this.version.Branch+'.txt" target="_blank">v' + remote_version.Major + '-' + remote_version.Minor + ' (' + remote_version.BuildID.substring(0,8) + ')</a></div>';
