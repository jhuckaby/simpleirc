...
my $script_url = "http://pixlcore.com/software/simpleirc/install-latest-$branch.txt";
...
my $temp_file = "/var/tmp/temp-upgrade-simpleirc-$branch-$$.sh";
...
while (defined($line = <$fh>)) {
	...
	last if $line =~ /SimpleIRC Daemon started/;
}
$fh->close();
